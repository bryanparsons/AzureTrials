...
function GetAllDocuments {
    ...
    $allDocs = Get-CosmosDbDocument -CollectionId $Global:CosmosContainer -QueryEnableCrossPartition $true -Query "SELECT c.id, c.targetUserType FROM c WHERE c.deactivationDateTime < '2020-12-31T00:00:00'" -Context $CosmosContext
    if ($null -eq $allDocs)
    {
        return ,@()
    }
